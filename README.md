...
IMGM 目前包含三个相互独立的使用场景：

- **本地图片库**：扫描本机目录，按文件名进行模糊搜索、分页浏览和复制。
- **AI 图片助手**：为本地图片建立文本向量和视觉向量，通过自然语言进行混合检索。
- **远程 RIR**：读取用户提供的远程 JavaScript 图片清单，浏览和复制远程图片；RIR 不使用本地图片库的列表和缓存。

## 1.1 数据和隐私说明

- 图片扫描和图片向量生成都在本地完成。
- SQLite 数据库、图片目录和模型缓存默认保存在本地 `data/` 或用户指定的位置。
- AI 检索时，用户的当前问题和必要的聊天历史会发送给设置中的 LLM 供应商，用于解析搜索意图和组织回答；本地图片文件不会因为普通 AI 搜索自动上传。
- LLM API 密钥通过桌面系统的安全存储能力加密后保存。
- `.env`、本地数据库、模型权重和个人图片不应提交到 Git。
...
源码位于 `src/`，通过 TypeScript 编译到 `dist/` 后运行。`electron-forge` 仍用于开发启动和兼容已有工程配置；正式发布包主要由 `electron-builder` 生成。
...
4. 一些用于测试的图片素材。
5. 如果使用 AI 功能，需要一个可用的 LLM API 配置。
...
如果 npm 下载速度较慢，可以配置其他 npm 镜像，或者安装 `cnpm` 后执行：
...
| 配置项 | 默认值 | 作用 |
| --- | --- | --- |
| `DEEPSEEK_API_KEY` | 空 | DeepSeek API 密钥，CLI/MCP 使用 |
...
桌面应用中的 LLM 配置优先通过“设置”完成。`.env` 主要用于命令行工具、MCP Server 或没有使用界面设置的运行场景。

如果 `IMAGE_DB_PATH` 指向的数据库不存在，IMGM 会自动创建目录、SQLite 数据库和空的 `images` 表，不会因为第一次打开 AI 功能就直接报“数据库不存在”。空数据库不会产生搜索结果，需要先建立图片索引。
...
如果修改了 `TEXT_MODEL_ID`、`IMAGE_MODEL_ID` 或模型缓存目录，需要重新执行模型下载，并使用对应模型重新建立图片索引。模型文件可能较大，`data/` 已被 Git 忽略，不建议把它们直接提交到仓库。

正式发布的安装包会在 GitHub Actions 中自动下载默认模型，并在打包时放入安装包的 `resources/models` 目录，普通用户不需要手动执行模型下载命令。
...
该命令会先编译 TypeScript，并检查 Electron 原生模块，然后启动桌面应用。
...
3. 点击“测试连通性”。测试不要求该配置已经激活。
4. 选择一个有效配置作为当前激活配置；同时最多只能激活一个配置。
5. 返回“图片库”，点击“选择文件夹”。
6. 点击“开始搜图”，确认图片可以正常扫描和浏览。
7. 点击“索引图片”，选择需要加入 AI 检索的图片并点击“应用”。
8. 等待右下角索引进度完成。
9. 在右侧 AI 图片助手中输入自然语言查询。

“开始搜图”和“索引图片”是两个不同操作：前者只负责扫描并显示图片，后者负责为图片生成向量并写入 SQLite，只有完成索引的图片才能被 AI 检索到。
...
使用 `electron-builder` 时，目标平台通常应在对应平台环境中构建。GitHub Actions 会自动并行生成：

- Windows：NSIS 安装器和 ZIP 便携包；
- Linux：`.deb` 和 `.rpm`；
- macOS：`.dmg` 和 ZIP。

发布流程位于 `.github/workflows/release.yml`。将包含该工作流的代码推送到远程后，创建 `v` 开头的标签即可触发自动打包：

```bash
git push origin 260827-pre
git tag v3.0.2
git push origin refs/tags/v3.0.2
```

工作流会自动下载模型、编译原生模块、打包各个平台并创建 GitHub Release，不需要手动为每个平台分别打包。
发布任务使用 GitHub Actions 自动生成的临时令牌，并在工作流中授予 `contents: write` 权限，通常不需要额外配置个人 `GH_TOKEN`。
...
1. 点击“选择文件夹”，选择图片根目录。
2. 点击“开始搜图”，程序会递归扫描目录及其子目录。
3. 扫描完成后，图片会显示在左侧图片区域。
4. 使用顶部搜索框按文件名进行模糊搜索。
5. 点击图片卡片上的复制按钮，将图片复制到系统剪贴板。
6. 使用分页按钮浏览其他图片。

普通图片库扫描的主要格式包括 `.jpg`、`.jpeg`、`.png` 和 `.gif`。图片索引扫描还支持 `.webp` 和 `.bmp`。

图片库每页显示 8 张图片。图片卡片包含缩略图和文件信息，鼠标悬停时会显示交互反馈。无法加载的图片会使用 404 占位图；404 占位图不会提供复制或改标签操作。
...
因此，后续进行 AI 搜索后，仍然可以通过历史按钮回到之前扫描的目录结果。
...
索引进度只显示当前文件名，不会把完整的本地路径全部展示出来。已使用过的扫描目录会保存在列表中，后续切换目录后仍可继续查看。

索引过程中不要删除或移动正在处理的图片。如果数据库路径指向其他参考项目，图片文件路径也必须仍然有效，否则数据库中虽然有记录，程序仍然无法读取图片。

## 4.4 404 占位图片和复制

当本地图片路径失效、远程图片加载失败或图片无法解码时，界面会显示 `public/img/404.png`。占位图仅用于提示，不具备复制、改标签等实际图片操作。

本地图片会使用 Electron 剪贴板写入。远程 RIR 图片会先下载并解码，再写入剪贴板：

- 普通图片写入图片数据；
- Windows 下识别到 GIF 动画时，会使用临时 GIF 文件和 Windows 文件剪贴板格式，尽量保留完整动画，而不是只复制第一帧；
- 远程图片必须能够被应用正常访问和解码，服务器返回 HTML、鉴权页面或不完整图片数据时会复制失败。

# 5.RIR拓展模块

RIR（Remote Image Retrieval）是一种远程图片资源约定。RIR 服务端提供一个 JavaScript 模块，模块中包含图片相对路径列表和图片资源根地址，IMGM 读取该模块后即可在桌面端分页查看和复制远程图片。
...
1. 点击左侧“远程 RIR”。
2. 在 RIR 输入框中填入远程 JavaScript 清单地址。
3. 点击解析按钮。
...
输入的应该是可以直接返回 RIR 模块内容的地址，例如：

```text
https://www.qidong.tech:5173/resource/pic/image_list.js
```

仅输入站点根地址并不能自动猜测清单文件名，除非服务端已经将根地址配置为返回 RIR 模块。

RIR 服务端需要满足以下条件：

- 使用 HTTPS 更适合正式使用；
- 返回内容是 ES Module，并通过 `export default` 导出对象；
- `target` 和 `list` 中的路径能够拼接成真实图片地址；
- JavaScript 文件的响应类型应为 `application/javascript` 或兼容的 JavaScript MIME 类型；
- 服务端应允许来自桌面应用的跨域模块和图片请求（CORS）；
- 如果使用自定义域名，需要确保应用的 Content Security Policy 允许该域名加载模块；
- 远程图片服务器应允许应用访问图片，并返回真实的图片内容，而不是登录页或防盗链提示页。
...
AI 图片助手位于主界面右侧。它不是简单地把最后一条聊天消息交给模型，而是按照“解析意图 → 本地检索 → 组织回答”的流程工作。
...
评测程序会读取 SQLite 图片索引和测试查询，输出 Precision@8 等结果。结果文件位于 `artifacts/evaluation/`。
...
在“设置 → 图片库配置”中可以设置全局搜索结果历史上限。该上限只控制搜索结果历史，不会删除图片文件和 SQLite 图片记录。
...
| 刷新 | `Ctrl+R` |
| 打开图片索引 | `Ctrl+I` |
| 上一个搜索结果 | `Alt+ArrowLeft` |
| 下一个搜索结果 | `Alt+ArrowRight` |
| 切换图片库/RIR | `Ctrl+Shift+M` |

快捷键可以在设置中重新录入。快捷键不能重复，保存时会进行校验。

## 8.3 RIR 设置

在“设置 → RIR 配置”中填写默认 RIR 地址。之后打开远程 RIR 模块时，输入框会自动填入该地址。

## 8.4 本地数据目录

| 路径 | 内容 |
| --- | --- |
| `data/app.db` | 本地图片索引数据库 |
| `data/models` | 源码运行时的模型缓存 |
| `data/dataset` | 图片元数据和数据集文件 |
| Electron 用户数据目录 | 设置、搜索历史和 RIR 列表 |
| 打包应用 `resources/models` | 发布版随安装包提供的默认模型 |

`data/` 已加入 `.gitignore`。清理该目录会删除本地数据库、模型和数据集，请在操作前做好备份。
...
## 11.2 提示 `tokenizer_class` 或模型加载失败
